...
import importlib
import numpy as np
from models import save_model, load_two_stream_model
from data.ChromosomeDataLoaders import ChromosomeDataLoader  # Updated import
from chromosome_train_utils import AverageMeter, load_config, splitprint, runid_checker, predict_dataloader, batch_eval


def parse_args():
    parser = argparse.ArgumentParser(description="Chromosome Classification Training")
    ...
    parser.add_argument("--num_workers", default=4, type=int, help="number of threads for sampling. (default: 4)")
...
def select_dataloader(modality):
    ...
    if modality == "mm":
        return ChromosomeDataLoader  # Updated to use ChromosomeDataLoader
    else:
...
def main(opts):
    ...
    for epoch in range(configs.train_params["max_epoch"]):
        ...
        for i, (inputs, labels_onehot, imagenames) in enumerate(train_loader):
            ...
            losses.update(loss.item(), inputs_size)  # Use loss.item() to get scalar value
